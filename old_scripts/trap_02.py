...
def initialize_datasets(train_folder, valid_folder, test_folder, transform):
    ...
    train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)  # Increase batch size to 64
...
class ASLClassifier(nn.Module):
    def __init__(self, num_classes, model_name='efficientnet_b0', dropout_rate=0.4):
        ...
        # Create the base model without the classifier head
        self.base_model = timm.create_model(model_name, pretrained=True, num_classes=0)  # num_classes=0 removes classifier

        # Get the number of features from the base model
        num_features = self.base_model.num_features

        # Create custom classifier with dropout BEFORE final layer
...
model = ASLClassifier(num_classes=num_classes, model_name=model_name, dropout_rate=dropout_rate)
...
optimizer = optim.Adam(model.parameters(), lr=0.0005, weight_decay=1e-4)  # Reduce learning rate and add weight decay for regularization

# Step 3.1: Define Learning Rate Scheduler
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=3, factor=0.3)  # Increase patience to 3 and reduce learning rate more aggressively

# Step 3.2: Training and Validation Loop with Early Stopping
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=25):  # Increase epochs to 25
    train_losses, val_losses = [], []
    best_val_loss = float('inf')
    no_improvement_epochs = 0
    patience = 4  # Increase patience to 4 epochs
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            no_improvement_epochs = 0
            torch.save(model.state_dict(), 'asl_classifier_best_model.pth')  # Save the best model with a new name
        else:
...
train_losses, val_losses = train_and_validate(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=25)
