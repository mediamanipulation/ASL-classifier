...
optimizer = optim.Adam(model.parameters(), lr=0.0005)  # Reduce learning rate for finer adjustments

# Step 3.1: Define Learning Rate Scheduler
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=3, factor=0.5)  # Increase patience to 3

# Step 3.2: Training and Validation Loop with Early Stopping
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=20):  # Increase epochs to 20
...
train_losses, val_losses = train_and_validate(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=20)
